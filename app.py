...
def main():
    ...
    if 'messages' not in st.session_state:
        # messages: 대화내역관리
        st.session_state['messages'] = [{'role': 'system', 'content': '당신은 친절한 챗봇입니다.'}]
    ...
    with col1:
        ...
        audio = audiorecorder()
        if audio:
            print(f"녹음길이: {audio.duration_seconds}s")
        ...
        if (audio.duration_seconds > 0) and (not st.session_state['check_reset']):
            ...
            st.session_state['messages'].append({'role': 'user', 'content': prompt}) # 사용자 질문 messages에 추가            
            response = ask_gpt(st.session_state['messages'], model) # - llm 요청 
            st.session_state['messages'].append({'role': 'assistant', 'content': response}) # AI 답변 messages에 추가
            print(f"response={response}")

            # 4) llm 응답을 tts 모델을 통해 음원파일로 변환 및 재생 
            ...
            ''') # <source .... /> 프리픽스? 
            # <audio controls> 태그를 넣으면 재생버튼 나옴 
            # <audio autoplay='true'> 자동재생
    ...
    with col2:
        ...
        if (audio.duration_seconds > 0) and (not st.session_state['check_reset']):
            ...
            st.session_state['check_reset'] = False # 초기화상태값 원복
